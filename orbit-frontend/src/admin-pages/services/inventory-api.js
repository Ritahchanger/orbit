...
export const inventoryApi = {
    ...
    getInventoryStats: async (storeId) => {
        try {
            const response = await api.get(`/stores-inventory/${storeId}/inventory/stats`);
            return response.data;
        } catch (error) {
            throw error;
        }
    ...
    getLowStockAlerts: async (storeId, limit = 20) => {
        try {
            const response = await api.get(`/stores-inventory/${storeId}/inventory/alerts`, {
                params: { limit },
            });
            return response.data;
        } catch (error) {
    ...
    getAvailableProducts: async (storeId, filters = {}) => {
        try {
            const response = await api.get(`/stores-inventory/${storeId}/inventory/available`, {
                params: filters,
            });
            return response.data;
        } catch (error) {
    ...
    getInventoryItem: async (inventoryId) => {
        try {
            const response = await api.get(`/stores-inventory/inventory/${inventoryId}`);
            return response.data;
        } catch (error) {
            throw error;
        }
    ...
    generateInventoryReport: async (storeId, format = "json", dateRange = {}) => {
        try {
            const response = await api.get(`/stores-inventory/${storeId}/inventory/report`, {
    ...
    quickAddBySku: async (storeId, sku, quantity = 1) => {
        try {
            ...
            const response = await api.post(`/stores-inventory/${storeId}/inventory/quick-add`, {
    ...
    updateInventoryItem: async (inventoryId, updateData) => {
        try {
            const response = await api.put(`/stores-inventory/${inventoryId}`, updateData);
            return response.data;
        } catch (error) {
            throw error;
        }
    ...
    removeFromInventory: async (inventoryId) => {
        try {
            const response = await api.delete(`/${inventoryId}`);
            return response.data;
        } catch (error) {
            throw error;
        }
    ...
    restockProduct: async (inventoryId, quantity) => {
        try {
            const response = await api.post(`/stores-inventory/inventory/${inventoryId}/restock`, {
                quantity,
            });
            return response.data;
        } catch (error) {
    ...
    recordSale: async (inventoryId, saleData) => {
        try {
            const response = await api.post(`/stores-inventory/${inventoryId}/sale`, saleData);
            return response.data;
        } catch (error) {
            throw error;
        }
    ...
    adjustStock: async (inventoryId, quantity, reason, note = "") => {
        try {
            const response = await api.post(`/stores-inventory/${inventoryId}/adjust`, {
                quantity,
                reason,
                note,
            });
    ...
    searchInventory: async (storeId, searchTerm, filters = {}) => {
        return inventoryApi.getStoreInventory(storeId, { search: searchTerm, ...filters });
    },
    ...
    getLowStockItems: async (storeId, filters = {}) => {
        return inventoryApi.getStoreInventory(storeId, { lowStock: true, ...filters });
    },
    ...
    getOutOfStockItems: async (storeId, filters = {}) => {
        return inventoryApi.getStoreInventory(storeId, { outOfStock: true, ...filters });
    },
...
export const inventoryHelpers = {
    ...
    formatInventoryItem: (item) => {
        return {
            ...
            needsRestock: (item.storeStock || item.stock) <= (item.storeMinStock || item.minStock),
