...
import { useInventoryStats, useLowStockAlerts, useStoreInventory } from '../../hooks/store-inventory.queries';
...
const InventoryPage = () => {
  ...
  const pagination = inventoryData?.pagination || { page: 1, limit: 20, total: 0, pages: 1 };
  ...
  const handleDelete = async (inventoryId) => {
    if (window.confirm('Are you sure you want to remove this item from inventory?')) {
      try {
        await inventoryOperations.removeFromInventoryAsync(inventoryId);
      } catch (error) {
        // Error is handled in the mutation
      }
    }
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'In Stock': return 'bg-green-500/10 text-green-400 border border-green-500/20';
      case 'Low Stock': return 'bg-yellow-500/10 text-yellow-400 border border-yellow-500/20';
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-4 sm:p-6">
          ...
          {tabs.map(tab => {
            ...
            return (
              <button
                ...
                disabled={!hasStore}
                className={`flex items-center gap-2 px-4 py-2.5 rounded-sm transition-colors font-medium text-sm whitespace-nowrap ${activeTab === tab.id
        ...
        {isError && (
          <div className="mb-6 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700/30 rounded-sm">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="font-medium text-red-700 dark:text-red-300 text-sm">Failed to load inventory data</p>
                <p className="text-gray-600 dark:text-gray-400 text-xs">Please check your connection and try again</p>
              </div>
              <button
                onClick={() => refetchInventory()}
        ...
        {isLoading && (
          <div className="mb-6 p-8 text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-3 text-sm">Loading inventory data...</p>
          </div>
        )}
