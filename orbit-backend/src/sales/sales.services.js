...
const recordSale = async (saleData) => {
  ...
  if (storeInventory.stock < saleData.quantity) {
    throw new Error(
      `Insufficient stock. Available: ${storeInventory.stock} units`,
    );
  }

  // Calculate totals
  const unitPrice =
    saleData.unitPrice || storeInventory.sellingPrice || product.price;
  const discount = saleData.discount || 0;
  const subtotal = unitPrice * saleData.quantity;
  const total = subtotal - discount;
  const profit =
    saleData.profit ||
    (unitPrice - (storeInventory.costPrice || product.costPrice)) *
      saleData.quantity;
...
const getDailySalesSummary = async (
  date = new Date(),
  storeId = null,
  businessId,
) => {
    ...
    data: {
      ...
      itemsSold: itemsSold.map((item) => ({
        ...
        profitMargin:
          item.buyingPrice > 0
            ? (
                ((item.averageSellingPrice - item.buyingPrice) /
                  item.buyingPrice) *
                100
              ).toFixed(1)
            : "0.0",
      ...
      metrics: {
        ...
        itemsPerTransaction:
          summaryData.totalTransactions > 0
            ? (
                summaryData.totalItemsSold / summaryData.totalTransactions
              ).toFixed(1)
            : "0.0",
      },
    },
  ...
  const enrichedProducts = await Promise.all(
    topProducts.map(async (product) => {
      ...
      return {
        ...
        image:
          productDetails?.images?.find((img) => img.isPrimary)?.displayUrl ||
          productDetails?.images?.[0]?.displayUrl,
        profitMargin:
          productDetails?.costPrice > 0
            ? (
                ((product.averageSellingPrice - productDetails.costPrice) /
    ...
    Sale.aggregate([
      ...
      },
      {
        $group: {
          ...
          transactionCount: { $sum: 1 },
          profitPerUnit: {
            $avg: { $subtract: ["$unitPrice", "$productDetails.costPrice"] },
          },
        },
      },
      {
        ...
        overallSummary: {
          ...
          profitMargin:
            overallSummary.totalRevenue > 0
              ? (
                  (overallSummary.totalProfit / overallSummary.totalRevenue) *
                  100
                ).toFixed(2)
              : "0.00",
        },
...
const getItemsSoldByDateRange = async (
  startDate,
  endDate,
  page = 1,
  limit = 50,
    ...
    Sale.aggregate([
      ...
      },
      {
        $project: {
          ...
          transactionCount: 1,
          profitPerUnit: {
            $subtract: ["$averageSellingPrice", "$buyingPrice"],
          },
          profitMargin: {
            $cond: [
              { $gt: ["$buyingPrice", 0] },
    ...
    data: {
      ...
      summary: {
        ...
        averageRevenuePerProduct:
          itemsSold.length > 0
            ? (
                itemsSold.reduce((sum, item) => sum + item.totalRevenue, 0) /
                itemsSold.length
              ).toFixed(2)
            : 0,
      },
