...
const { findProducts, countProducts } = require("../products");
...
class ReportService {
  ...
  async getDashboardStats(storeId = null, businessId) {
    try {
      ...
      const [
        ...
        this.getStoreCount(bizId),
        this.getProductCount(bizId),
        ...
        Sale.find({
          ...
          .select(
            "productName quantity total paymentMethod customerName saleDate",
          ),
      ...
      return {
        ...
        topProducts: Array.isArray(topSellingProducts)
          ? topSellingProducts
          : [],
        ...
        storeStatus: storeId
          ? storeStatus
          : this.formatStoreStatus(storeStatus || []),
      };
    } catch (error) {
   ...
   */
  async getSalesReport(filters = {}, businessId) {
    try {
      const {
        startDate,
        endDate,
      ...
      const matchStage = { status, businessId };
  ...
  async getMonthlySalesSummary({ year, month, storeId, businessId }) {
   ...
   */
  async getInventoryReport(filters = {}, businessId) {
    try {
      const {
        storeId,
        category,
      ...
      // Match stage
      const matchStage = {};
      if (businessId) matchStage.businessId = new mongoose.Types.ObjectId(businessId);
      if (storeId) matchStage.store = new mongoose.Types.ObjectId(storeId);
      if (status) matchStage.status = status;
      if (minStock !== undefined)
        matchStage.stock = { ...matchStage.stock, $gte: minStock };
      if (maxStock !== undefined)
        matchStage.stock = { ...matchStage.stock, $lte: maxStock };
      ...
      summaryPipeline.push({
        $group: {
          ...
          totalCostValue: {
            $sum: { $multiply: ["$stock", "$product.costPrice"] },
          },
          lowStockCount: {
            $sum: { $cond: [{ $eq: ["$status", "Low Stock"] }, 1, 0] },
          },
      ...
      return {
        ...
        summary: {
          ...
          profitMargin:
            summaryData.totalCostValue > 0
              ? ((summaryData.totalValue - summaryData.totalCostValue) /
                  summaryData.totalCostValue) *
                100
              : 0,
   ...
   */
  async getProductPerformanceReport(filters = {}, businessId) {
    const { startDate, endDate, storeId, category, limit = 20 } = filters;
    ...
    let productMatch = {};
    if (category) {
      const products = await findProducts({ category }, { select: "_id" });
    ...
    const productIds = salesData.map(
    ...
    const mergedData = salesData.map((sale) => {
      ...
      return {
        ...
        stockTurnover:
          sale.totalSold > 0 && currentStock > 0
            ? sale.totalSold / currentStock
            : 0,
        ...(storeId && {
   ...
   */
  async getStorePerformanceReport(filters = {}, businessId) {
    try {
      const {
        startDate,
        endDate,
      ...
      throw new Error(
        `Failed to get store performance report: ${error.message}`,
      );
    }
  }
   ...
   */
  async getSalesTrendReport(period = "daily", filters = {}, businessId) {
    try {
  ...
  async getFinancialSummary(filters = {}) {
    try {
      const { startDate, endDate, storeId, businessId } = filters;
      ...
      const [salesSummary, paymentMethods, topProducts, dailyBreakdown] =
        await Promise.all([
  ...
  // Helper methods
  async getStoreCount(businessId) {
    const filter = { status: "active" };
    if (businessId) filter.businessId = new mongoose.Types.ObjectId(businessId);
    return Store.countDocuments(filter);
  }

  async getProductCount(businessId) {
    const filter = { status: { $in: ["active", "In Stock", "Low Stock"] } };
    if (businessId) filter.businessId = new mongoose.Types.ObjectId(businessId);
   ...
   */
  async exportReport(type, filters = {}, format = "csv", businessId) {
    try {
      let data;

      switch (type) {
        case "sales":
          data = await this.getSalesReport(filters, businessId);
          break;
        case "inventory":
          data = await this.getInventoryReport(filters, businessId);
          break;
        case "products":
          data = await this.getProductPerformanceReport(filters, businessId);
          break;
        case "stores":
          data = await this.getStorePerformanceReport(filters, businessId);
          break;
        case "dashboard":
          data = await this.getDashboardStats(filters.storeId, businessId);
          break;
        case "financial":
          data = await this.getFinancialSummary({ ...filters, businessId });
          break;
        default:
          throw new Error(
            `Invalid export type. Valid types: sales, inventory, products, stores, dashboard, financial`,
          );
      }
  ...
  formatDashboardForCSV(data) {
    ...
    const rows = [
      ...
      [
        "Inventory Value",
        `$${data.overview.inventoryValue?.toFixed(2) || "0.00"}`,
        "",
      ],
      ["Today Sales Count", data.salesSummary.today.count, ""],
      [
        "Today Revenue",
        `$${data.salesSummary.today.revenue?.toFixed(2) || "0.00"}`,
        "",
      ],
      [
        "Today Profit",
        `$${data.salesSummary.today.profit?.toFixed(2) || "0.00"}`,
        "",
      ],
      ["Monthly Sales Count", data.salesSummary.thisMonth.count, ""],
      [
        "Monthly Revenue",
        `$${data.salesSummary.thisMonth.revenue?.toFixed(2) || "0.00"}`,
        "",
      ],
      [
        "Monthly Profit",
        `$${data.salesSummary.thisMonth.profit?.toFixed(2) || "0.00"}`,
        "",
      ],
  ...
  formatFinancialForCSV(data) {
    ...
    const rows = [
      ...
      [
        "Total Profit",
        `$${data.summary.totalProfit?.toFixed(2) || "0.00"}`,
        `${data.summary.profitMargin?.toFixed(2) || "0.00"}%`,
        "",
      ],
      ["Total Items Sold", data.summary.totalItemsSold || 0, "", ""],
      [
        "Average Transaction",
        `$${data.summary.avgTransactionValue?.toFixed(2) || "0.00"}`,
        "",
        "",
      ],
      [
        "Avg Profit/Transaction",
        `$${data.summary.avgProfitPerTransaction?.toFixed(2) || "0.00"}`,
        "",
        "",
      ],
    ];
    ...
    data.paymentMethods.forEach((pm) => {
      const percentage = (
        (pm.amount / data.summary.totalRevenue) *
        100
      ).toFixed(2);
      rows.push([
        `Payment: ${pm._id}`,
  ...
  async createSalesExcel(workbook, data) {
    ...
    if (data.summary) {
      ...
      const summaryData = [
        ...
        [
          "Average Sale:",
          `$${data.summary.averageSaleValue?.toFixed(2) || "0.00"}`,
        ],
  ...
  async createInventoryExcel(workbook, data) {
    ...
    if (data.summary) {
      ...
      const summaryData = [
        ...
        [
          "Total Cost:",
          `$${data.summary.totalCostValue?.toFixed(2) || "0.00"}`,
        ],
        [
          "Potential Profit:",
          `$${data.summary.potentialProfit?.toFixed(2) || "0.00"}`,
        ],
        [
          "Profit Margin:",
          `${data.summary.profitMargin?.toFixed(2) || "0.00"}%`,
